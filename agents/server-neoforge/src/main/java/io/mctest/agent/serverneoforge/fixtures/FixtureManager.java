package io.mctest.agent.serverneoforge.fixtures;
...
import io.mctest.agent.serverneoforge.Params;
 ...
 *
 * <p>Every game access routes through {@link Names} (the only Mojmap-mapped file) on the server thread;
 * the SUT provider is discovered via {@link Names#lookupFixtureProvider()} (a
 * {@link java.util.ServiceLoader} lookup — the NeoForge discovery mechanism). Each applied fixture records
 ...
 *
 * <p>Vanilla NeoForge has no Bukkit-style per-player permission API, so the {@code permissions} built-in
...
public final class FixtureManager {
    ...
    public JsonObject reset(McTestSession session, JsonObject params) throws McTestException {
        ...
        if (snapshot != null) {
            throw McTestException.fixtureFailed(
                    "Snapshot restore is not supported by the neoforge agent: " + snapshot);
        }
    ...
    private Applied applyBuiltIn(String fixture, Map<String, Object> args) throws McTestException {
        ...
        switch (fixture) {
            ...
            case "permissions":
                // No vanilla NeoForge per-player permission API; honest failure unless a SUT provider
                // claimed it (handled before we reach the built-in path).
                throw McTestException.fixtureFailed(
                        "permissions fixture is not supported by the neoforge agent (no provider registered)");
            default:
