...
import io.mctest.agent.serverforge.Params;
 ...
 * {@link java.util.ServiceLoader} lookup — the Forge discovery mechanism). Each applied fixture records
 ...
 *
 * <p>Vanilla Forge has no Bukkit-style per-player permission API, so the {@code permissions} built-in
...
public final class FixtureManager {
    ...
    public JsonObject reset(McTestSession session, JsonObject params) throws McTestException {
        ...
        if (snapshot != null) {
            throw McTestException.fixtureFailed(
                    "Snapshot restore is not supported by the forge agent: " + snapshot);
        }
    ...
    private Applied applyBuiltIn(String fixture, Map<String, Object> args) throws McTestException {
        ...
        switch (fixture) {
            ...
            case "permissions":
                // No vanilla Forge per-player permission API; honest failure unless a SUT provider
                // claimed it (handled before we reach the built-in path).
                throw McTestException.fixtureFailed(
                        "permissions fixture is not supported by the forge agent (no provider registered)");
            default:
