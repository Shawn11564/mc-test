...
import io.mctest.agent.clientfabric.mappings.Names;
 ...
 * client starts, hosts an MCTP WebSocket server and exposes the real client's Screen/widget tree,
 * keyboard, and framebuffer to the runner's in-process driver (DRIVERS.md §2). It is a thin shim — all
 * wire logic, negotiation, selector resolution and error mapping live in the shared
 * {@code io.mctest.agent.core} ({@code MctpServer} + {@code Dispatch} + {@code ScreenHandlers}); this
 * class only assembles them around a loader-specific {@link ClientBridge}.
 *
 * <p><b>Mappings quarantine.</b> This entrypoint imports ONLY the shared core and the Fabric loader
 * entrypoint API ({@code net.fabricmc.api.ClientModInitializer}). Every Yarn-mapped Minecraft symbol is
 ...
 */
public final class McTestClientMod implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // The loader-specific bridge — the ONLY object that touches Yarn-mapped client internals. Kept
        // as the concrete type so we can read the version strings it derives from the client runtime
        // (those getters are loader-specific, NOT part of the loader-neutral ClientBridge interface).
        Names bridge = new Names();

        // EventBus wiring (see the wiring note in M4_PLAN.scratch.md §B): the loader-neutral
