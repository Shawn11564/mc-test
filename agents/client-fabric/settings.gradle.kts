// mc-test client-fabric agent — STANDALONE Gradle build (NOT part of agents/settings.gradle.kts).
//
// This shim is a Fabric Loom mod that needs the Minecraft toolchain + network (Yarn mappings, the
// Fabric loader/API, a real client). It is therefore kept OUT of the `mc-test-agents` build so that
...
//
// ACCEPTANCE-ONLY in this repo's CI: this build is NOT run here (no Loom, no network). It is written
// to be correct against the contract; the CI-provable client logic lives in /agents/core (the
...
rootProject.name = "mc-test-client-fabric"
