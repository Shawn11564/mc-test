// mc-test client-fabric agent — Fabric Loom BUILD CONFIG.
//
// !!! ACCEPTANCE-ONLY — NOT BUILT IN THIS REPO'S CI. !!!
// Fabric Loom downloads Minecraft + Yarn mappings and needs the network + a real client runtime, so
// this module is a STANDALONE Gradle build kept out of `agents/settings.gradle.kts`. The CI-provable
// half of the client agent (the loader-neutral screen logic) lives in /agents/core and is exercised
// there with a FakeClientBridge (no Minecraft). This shim only supplies the entrypoint and the
// Yarn-mapped `mappings/Names.java` ClientBridge impl.
//
...
base {
    ...
    archivesName.set("agent-client-fabric")
}

// Drop the version suffix from ALL archive tasks (jar + Loom's remapJar) so the produced jar is exactly
// `agent-client-fabric.jar` — the name the runner's KNOWN_CLIENT_AGENTS resolves (cli.ts). project.version
...
dependencies {
    ...
    // Shaded into the mod jar via `include(...)` so the running client carries it without a separate jar.
