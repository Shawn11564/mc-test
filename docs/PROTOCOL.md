...
- **Release contract.** This section *is* the wire compatibility contract for releases. A published release MUST NOT make a backward-incompatible wire change (remove/rename a method, param, result field, capability key, or error code; change an existing field's meaning) without a **major** `protocolVersion` bump and a new sub-protocol token (`mctp.v2`, …). Within a major, releases may only **add** (optional params, result fields, methods, capability keys, error codes), and every client/agent keeps ignoring unknown optional inputs and unknown result/event fields. Because the runner and every driver/agent exchange `protocolVersion`/`supportedProtocols` at handshake, an incompatible major is a clean `-32099 PROTOCOL_VERSION_UNSUPPORTED` at session creation — never a silent behavior change mid-session. Bumping the wire contract is therefore a deliberate, documented release event, gated by the golden conformance fixtures (`packages/protocol/fixtures/conformance/`).
