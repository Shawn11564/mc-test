...
**Server driver registered (F5, `server`).** The cost-1 **`server`** driver (driver id `server`, MCTP `agent.kind: serverPlugin` / `serverMod`) — formerly described only as the cheapest co-driver *tier* — is now a **registered, selectable primary driver** in the runner's `DriverRegistry` at **cost 1** (the cheapest, per [§7](#7-driver-selection-algorithm)). It runs a **server-truth-only session**: the co-selected server agent **is** the primary connection and **no player joins** (`world.join`/`world.leave` are no-ops in such a session). The runner selects it standalone when a test's top-level `requires` names **only** server-owned caps (e.g. `pluginState` / `worldTruth` / `fixtures` — the canonical `regions.modloaded` test requires only `pluginState`). For **selection** the `server` driver advertises `worldTruth, pluginState, fixtures, fakePlayers` (all loaders; `mcVersionRange: ">=1.8"`); it has **no MCTP endpoint of its own** — the real session and advertised set come from the co-selected server agent (`server-bukkit` / `server-fabric` / `server-forge` / `server-neoforge`). **Rationale (why server-truth-only matters):** a Mineflayer bot can connect to a *Fabric* server but **cannot** complete Forge/NeoForge's FML login handshake, so the **only** way to assert on a Forge/NeoForge server is this no-player, server-truth path. When the `server` driver is selected but **no** server agent is co-selected for the target, the test is **`skipped`** with the runner-level reason **`NO_SERVER_AGENT`** (category `environment`, [§8](#8-skip-with-reason-semantics)) — an honest skip, never a false pass. (All three server truth agents — `server-fabric`, `server-forge`, `server-neoforge` — are built and boot a real dedicated server to answer `mod.loaded` over MCTP; a target still honest-skips `NO_SERVER_AGENT` when its server agent jar is not present — see `DRIVERS.md` §3.)
...
| Required co-driver (server agent) absent for this target, **or** the `server` driver selected (server-truth-only) with no server agent co-selected | handshake / selection | `environment` (reason `NO_SERVER_AGENT`) |
...
`skip.category` ∈ `{ capability, target, environment, configuration, manual }`. `skip.reason` is the runner-level token, e.g. `NO_COMPATIBLE_DRIVER` for the `capability` category, or `NO_SERVER_AGENT` for the `environment` category (the cost-1 `server` driver was selected for a server-truth-only session but no server agent was co-selected — see [§4](#4-driver--capability-matrix)).
