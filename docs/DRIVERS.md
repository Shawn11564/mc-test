...
> **F4 update (2026-06-16) — the launcher is loader-aware (multi-loader fan-out).** As of **F4** the launch path resolves **per loader** through a loader-aware resolver in **`packages/driver-inprocess/src/launch/loaders.ts`** (see §2.4): **Fabric/Quilt** use the simple `KnotClient` classpath launch from F3 (fully implemented, run on a real box); **Forge/NeoForge** use a *modular* launch — the loader **installer** is downloaded + run to produce a launcher profile (a vanilla-style version JSON that `inheritsFrom` the vanilla version, with `arguments.jvm`/`arguments.game` templates + a `cpw.mods.bootstraplauncher.BootstrapLauncher` main class), and the client boots as `java <substituted jvm args> -cp <all libs> cpw.mods.bootstraplauncher.BootstrapLauncher <substituted game args>`. The **pure** parts of the modular launch (installer-coordinate resolution, `${...}` placeholder substitution, OS-rule argument flattening, merging the loader profile onto vanilla, the modular `buildClientLaunch` branch) are **unit-tested offline** (`packages/driver-inprocess/test/loaders.test.ts`, `launch.test.ts`, `provisioner.test.ts`). The genuinely **CI-gated** part — running the loader installer + booting a forge/neoforge client on a GL-capable host — is honest about its status: by **default** (no opt-in) a forge/neoforge `inprocess` target **honest-SKIPS** with reason `UNSUPPORTED_TARGET` (never a crash, never a false green), and runs for real **only** when opted in via the env var **`MC_TEST_RENDERED_LOADERS=<loader>`** (comma-separated, e.g. `forge,neoforge`) on a capable runner — mirroring how F3's Fabric rendered-green is CI-gated. (Correctness note: **before F4** a forge/neoforge inprocess target threw `UNSUPPORTED_LOADER`, which surfaced as a **RED** failure — a false red; it is now an honest **SKIP**.) The pinned loader version is threaded from the target via the new `mc-test.yml` per-target field **`loaderVersion`** (Fabric/Quilt: optional, else newest stable for `mc`; Forge/NeoForge: **required** to run the modular installer launch — e.g. forge `"47.2.0"`, neoforge `"21.1.66"`) — see [`ENVIRONMENTS.md`](./ENVIRONMENTS.md). Build-artifact names are unchanged (`agent-client-forge.jar`, `agent-client-neoforge.jar`; §2.3).
...
- JDK 17 (MC 1.17+) / JDK 8 (legacy ≤1.16) per target; Gradle with Fabric Loom / ForgeGradle / NeoGradle. As of **F3** the Fabric jars **build** (Loom 1.7.4, Gradle 8.10.2, JDK 21): the regions Fabric mod (`examples/regions/mod-fabric`) → **`openregions-fabric.jar`**, `/agents/client-fabric` → **`agent-client-fabric.jar`** (shaded `/agents/core` + Java-WebSocket as jar-in-jar). The per-version Yarn mappings stay quarantined in `mappings/Names.java` (§2.3). The same OpenRegions SUT is also built for Forge (`mod-forge`, MC 1.20.1) and NeoForge (`mod-neoforge`, MC 1.21.1), so every loader row drives a real SUT.
- A **real Minecraft client** launched with the SUT mod **and** the matching `client-*` agent jar in `mods/`. `/packages/driver-inprocess` builds the launch itself (no external launcher): **`ClientProvisioner.ts`** resolves the Mojang version manifest → version JSON → downloads the client jar + libraries + (optional) asset bundle and the Fabric loader profile (meta.fabricmc.net) → loader libraries, extracts the LWJGL natives via a dependency-free ZIP reader, and stages a per-instance `gameDir/mods/` (SUT mod + client agent) from a content-addressed cache; **`ClientLauncher.ts`** emits the launch invocation.
- **Loader-aware launch (F4).** The launch is resolved **per loader** by `launch/loaders.ts`:
  - **Fabric / Quilt** → the F3 path: a real `java -Djava.library.path=<natives> -cp <all jars> net.fabricmc.loader.impl.launch.knot.KnotClient` offline invocation (username `Tester`, zero UUID, `--accessToken 0`, no Microsoft auth).
  - **Forge / NeoForge** → a **modular** launch: the loader **installer** (a Java program) is downloaded + run to produce a launcher profile (a vanilla-style version JSON that `inheritsFrom` the vanilla version, with `arguments.jvm`/`arguments.game` templates + a `cpw.mods.bootstraplauncher.BootstrapLauncher` main class); the client then boots as `java <substituted jvm args> -cp <all libs> cpw.mods.bootstraplauncher.BootstrapLauncher <substituted game args>`. The pure launch-assembly (installer-coordinate resolution, `${...}` substitution, OS-rule argument flattening, merging the loader profile onto vanilla, the modular `buildClientLaunch` branch) is **unit-tested offline** (`test/loaders.test.ts`, `launch.test.ts`, `provisioner.test.ts`).
  - The pinned loader version comes from the target's `loaderVersion` (`ENVIRONMENTS.md`): optional for Fabric/Quilt (else newest stable for `mc`), **required** for Forge/NeoForge to run the modular installer launch.
- **Forge/NeoForge live boot is opt-in (honest-skip by default).** Actually running the loader installer + booting a forge/neoforge client requires a GL-capable host, so it is **CI-gated** like F3's Fabric rendered-green: by **default** a forge/neoforge `inprocess` target **honest-SKIPS** with reason `UNSUPPORTED_TARGET` (never a crash, never a false green), and runs for real **only** when opted in via **`MC_TEST_RENDERED_LOADERS=<loader>`** (comma-separated, e.g. `forge,neoforge`) on a capable runner. (Before F4 this surfaced as a `UNSUPPORTED_LOADER` **RED** failure — a false red — now corrected to an honest SKIP.)
