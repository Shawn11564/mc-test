...
/** Detected/declared values that fill a generated CI workflow (docs/CI.md). */
interface CiOpts {
  engineRepo: string;
  engineRef: string;
  java: string;
...
/**
 * The thin caller workflow (default): references the reusable workflow in the engine
 * repo, so engine updates flow automatically and the consumer file stays ~15 lines.
 * Requires the engine repo to be reachable from Actions (public, or same-owner with
 * reusable-workflow access). In the template, `\${{ }}` escapes a GitHub expression so
 * JS does not treat it as interpolation.
 */
function ciCallerWorkflow(o: CiOpts): string {
  return `# Generated by \`mc-test init-ci\` — see docs/CI.md. Safe to edit + commit.
#
# Builds your plugin/mod, runs your .mctest.yml tests across mc-test.yml, and publishes
# the HTML report (${o.matrix === "mc-test.yml" ? "mc-test-report/report.html" : "the report"})
...
jobs:
  mc-test:
    uses: ${o.engineRepo}/.github/workflows/mc-test-ci.yml@${o.engineRef}
    with:
      java-version: '${o.java}'
      node-version: '${o.node}'
      build-command: '${yamlSq(o.buildCommand)}'
      tests: '${yamlSq(o.testsGlob)}'
      targets: '${yamlSq(o.targets)}'
      matrix: '${yamlSq(o.matrix)}'
      ...
      agents: '${yamlSq(o.agents)}'
      pages: true
`;
}

/**
 * The `--standalone` workflow: fully self-contained, no dependency on the engine repo's
 * reusable workflow (it clones + builds the engine inline). Bigger, but pins everything
 * and survives the engine repo being private/unavailable. Mirrors mc-test-ci.yml.
 */
function ciStandaloneWorkflow(o: CiOpts): string {
  return `# Generated by \`mc-test init-ci --standalone\` — see docs/CI.md. Safe to edit + commit.
#
# Self-contained: clones + builds the mc-test engine inline (no reusable-workflow
# dependency), builds your plugin/mod, runs your tests, and publishes the HTML report
# as a workflow artifact + to GitHub Pages (one-time: Settings → Pages → Source:
# "GitHub Actions"; the artifact is always uploaded regardless).
name: mc-test
...
jobs:
  run:
    name: Build SUT + run mc-test
    runs-on: ubuntu-latest
    timeout-minutes: 60
    steps:
      - name: Checkout project (SUT)
        uses: actions/checkout@v4
      - name: Checkout mc-test engine
        uses: actions/checkout@v4
        with:
          repository: ${o.engineRepo}
          ref: ${o.engineRef}
          path: .mc-test-engine
      ...
      - uses: actions/setup-node@v4
        with:
          node-version: '${o.node}'
          cache: npm
          cache-dependency-path: .mc-test-engine/package-lock.json
      - uses: gradle/actions/setup-gradle@v4
      - name: Build mc-test engine
        working-directory: .mc-test-engine
        run: |
          npm ci
      ...
      - name: Build agent jars (best-effort)
        working-directory: .mc-test-engine
        run: |
          gradle -p agents :core:publishToMavenLocal
          for agent in ${o.agents}; do
            echo "::group::build agent $agent"
            gradle -p agents ":\${agent}:build" || echo "::warning::agent '$agent' failed to build — its steps will honest-skip"
            echo "::endgroup::"
          done
      - name: Build SUT
        run: ${o.buildCommand}
      - name: Run mc-test
        env:
          TESTS_GLOB: '${yamlSq(o.testsGlob)}'
          TARGETS: '${yamlSq(o.targets)}'
          MATRIX: '${yamlSq(o.matrix)}'
        run: |
          shopt -s globstar nullglob
          FILES=( $TESTS_GLOB )
          if [ \${#FILES[@]} -eq 0 ]; then echo "::error::no step files matched '$TESTS_GLOB'"; exit 1; fi
          echo "Running \${#FILES[@]} step file(s): \${FILES[*]}"
          node .mc-test-engine/packages/runner/dist/cli.js run "\${FILES[@]}" \\
            --target "$TARGETS" --matrix "$MATRIX" --out mc-test-report
      ...
      - name: Upload Pages artifact
        if: \${{ always() && hashFiles('mc-test-report/**') != '' }}
  ...
  publish:
    ...
    needs: run
    if: \${{ always() && needs.run.result != 'cancelled' }}
    runs-on: ubuntu-latest
...
function yamlSq(s: string): string {
  return s.replace(/'/g, "''");
}

/**
 * `mc-test init-ci` — scaffold a GitHub Actions workflow into the consuming project
 * that builds the SUT, runs the .mctest.yml tests across the matrix, and publishes
 * report.html as a workflow artifact + to GitHub Pages. Never overwrites an existing
 * workflow. Defaults are detected from the project (Gradle vs Maven, tests dir); flags
 * override. `--standalone` emits a self-contained workflow instead of the thin caller.
 */
function cmdInitCi(args: Args): number {
  const dir = resolve(args.flags["dir"] ?? ".");
  const hasGradlew = existsSync(join(dir, "gradlew")) || existsSync(join(dir, "gradlew.bat"));
  const hasPom = existsSync(join(dir, "pom.xml"));
  const o: CiOpts = {
    engineRepo: args.flags["engine-repo"] ?? "Shawn11564/mc-test",
    engineRef: args.flags["engine-ref"] ?? "main",
    java: args.flags["java"] ?? "21",
    node: args.flags["node"] ?? "20",
    buildCommand: args.flags["build-command"] ?? (hasPom && !hasGradlew ? "mvn -B package" : "./gradlew build"),
    testsGlob:
      args.flags["tests"] ??
      (existsSync(join(dir, "mctests")) ? "mctests/**/*.mctest.yml" : "src/mctest/**/*.mctest.yml"),
    agents: args.flags["agents"] ?? "server-bukkit",
    matrix: args.flags["matrix"] ?? "mc-test.yml",
    targets: args.flags["target"] ?? "all",
  };
  const standalone = args.flags["standalone"] === "true";
  const content = standalone ? ciStandaloneWorkflow(o) : ciCallerWorkflow(o);
  const wfPath = join(dir, ".github", "workflows", "mc-test.yml");

  console.log(`mc-test init-ci${standalone ? " (standalone)" : ""}:`);
  console.log(`  ${scaffold(wfPath, content, ".github/workflows/mc-test.yml")}`);
  console.log(`  build command: ${o.buildCommand}`);
  console.log(`  step files:    ${o.testsGlob}`);
  console.log(`  agents:        ${o.agents}`);
  if (!standalone) console.log(`  engine:        ${o.engineRepo}@${o.engineRef} (reusable workflow)`);
  console.log("\nNext:");
  console.log("  1. Review .github/workflows/mc-test.yml (agents/targets/build command).");
  console.log("  2. To host the report on GitHub Pages: repo Settings → Pages → Source: \"GitHub Actions\".");
  console.log("     (The report is always uploaded as a downloadable artifact too — Pages is optional.)");
  console.log("  3. Commit + push. Each push/PR builds your tests and publishes report.html.");
  console.log("\nSee docs/CI.md.");
  return 0;
}
...
commands:
  ...
  init-ci                               scaffold a GitHub Actions workflow that runs your
                                        tests + publishes report.html [--dir <dir>]
        [--standalone]                  self-contained workflow (no reusable-workflow dep)
        [--build-command <cmd>] [--tests <glob>] [--agents "a b"] [--engine-ref <ref>]

docs: docs/GETTING_STARTED.md · docs/AUTHORING.md · docs/CI.md`;
